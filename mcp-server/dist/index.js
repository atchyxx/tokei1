...
import { visitPage } from './tools/visit.js';
import { parseFiles, parseDirectory } from './tools/file-parser.js';
import { embed, embedBatch, similarity, semanticSearch, } from './tools/embedding.js';
import { setActiveProject, getProjectInfo, detectLatestProject, } from './tools/project.js';
import { savePrompt, saveResearch, saveResearchJson, } from './tools/save.js';
// Tool definitions
const TOOLS = [
    ...
    try {
        switch (name) {
            ...
            case 'set_project': {
                ...
                const autoDetect = args?.autoDetect;
                let targetPath;
                if (autoDetect) {
            ...
            case 'embed': {
                ...
                if (Array.isArray(textInput)) {
                    const results = await embedBatch(textInput);
                    return {
                        content: [
                            {
                                type: 'text',
                    ...
                    const result = await embed(textInput);
                    return {
                        content: [
                            {
                                type: 'text',
            ...
            case 'similarity': {
                const textA = args?.textA;
                const textB = args?.textB;
                const result = await similarity(textA, textB);
                return {
                    content: [
                        {
                            type: 'text',
