 ...
 * デフォルト設定
 */
export const DEFAULT_CONTENT_VALIDATION_CONFIG = {
    minLength: 100,
    warningLength: 500,
 ...
 */
export function calculateMeaningfulRatio(content) {
...
export function detectContentType(content) {
    ...
    const errorCount = errorIndicators.filter((indicator) => lower.includes(indicator)).length;
    if (errorCount >= 2) {
        return 'error_page';
    }
    // ログインページの検出
    ...
    const loginCount = loginIndicators.filter((indicator) => lower.includes(indicator)).length;
    if (loginCount >= 3) {
        return 'login_page';
    }
    // HTMLの検出
