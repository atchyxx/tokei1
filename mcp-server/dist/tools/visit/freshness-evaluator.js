 ...
 * デフォルトオプション
 */
export const DEFAULT_FRESHNESS_OPTIONS = {
 ...
 * トピック別の鮮度閾値
 */
export const TOPIC_FRESHNESS_THRESHOLDS = {
    // 急速に変化するトピック
...
export function extractDateFromHTML(html) {
    ...
    if (!publishDate) {
        for (const name of DATE_PATTERNS.meta.names) {
            const metaMatch = html.match(new RegExp(`<meta[^>]+(?:name|property)=["']${name}["'][^>]+content=["']([^"']+)["']`, 'i'));
            if (metaMatch) {
...
export function extractDateFromText(text) {
    ...
    if (updatedMatch) {
        ...
        if (date)
            return { date, rawText: updatedMatch[0] };
    }
    // 公開日表記
    ...
    if (publishedMatch) {
        ...
        if (date)
            return { date, rawText: publishedMatch[0] };
    }
    // 日本語日付
    ...
    if (isoMatch) {
        ...
        if (!isNaN(date.getTime()))
            return { date, rawText: isoMatch[0] };
    }
    // 英語日付
...
function parseEnglishDate(text) {
    const match = text.match(/(January|February|March|April|May|June|July|August|September|October|November|December|Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+(\d{1,2}),?\s+(\d{4})/i);
    if (match) {
 ...
 * 鮮度レベルを計算
 */
export function calculateFreshnessLevel(daysOld, options) {
    if (daysOld < 0)
        return 'unknown';
    if (daysOld <= options.freshThresholdDays)
        return 'fresh';
    if (daysOld <= options.recentThresholdDays)
        return 'recent';
    if (daysOld <= options.staleThresholdDays)
        return 'stale';
    return 'outdated';
}
/**
 * 鮮度スコアを計算（0-100）
 */
export function calculateFreshnessScore(daysOld, options) {
    if (daysOld < 0)
        return 0;
    if (daysOld === 0)
        return 100;
 ...
 * 鮮度を評価
 */
export function evaluateFreshness(content, options) {
    ...
    const daysOld = Math.floor((effectiveOptions.referenceDate.getTime() - publishDate.getTime()) / msPerDay);
    // 鮮度レベルとスコアを計算
...
export class FreshnessEvaluator {
     ...
     * トピックに基づいた鮮度評価
     */
    evaluateWithTopic(content, topic) {
     ...
     * トピックの閾値を取得
     */
    static getTopicThresholds(topic) {
...
//# sourceMappingURL=freshness-evaluator.js.map