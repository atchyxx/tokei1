...
export class PatentSearchOptimizer {
     ...
     * キーワードから分類コードを推定
     */
    suggestClassifications(keywords) {
        const results = [];
        const keywordsLower = keywords.map((k) => k.toLowerCase());
        for (const mapping of this.classificationMappings) {
            const matchCount = mapping.keywords.filter((kw) => keywordsLower.some((k) => k.includes(kw.toLowerCase()) || kw.toLowerCase().includes(k))).length;
            if (matchCount > 0) {
        ...
        return results
            ...
            .filter((item, index, self) => index === self.findIndex((t) => t.system === item.system && t.code === item.code))
            .slice(0, 10);
    }
    /**
     * キーワードを特許検索用に展開
     ...
     * 特許庁ごとの検索クエリを生成
     */
    generateOfficeQuery(query, office) {
     ...
     * JPO向けクエリ生成
     */
    generateJPOQuery(query, classifications) {
        const parts = [];
        // キーワード
     ...
     * USPTO向けクエリ生成
     */
    generateUSPTOQuery(query, classifications) {
        const parts = [];
        // キーワード
        if (query.keywords.length > 0) {
     ...
     * EPO向けクエリ生成
     */
    generateEPOQuery(query, classifications) {
        const parts = [];
        // キーワード
        if (query.keywords.length > 0) {
     ...
     * WIPO向けクエリ生成
     */
    generateWIPOQuery(query, classifications) {
        const parts = [];
        // キーワード
        if (query.keywords.length > 0) {
     ...
     * 検索URLを生成
     */
    generateSearchUrl(query, office) {
    ...
    optimizeQuery(query) {
        ...
        const allClassifications = [
            ...
            ...suggestedClassifications.filter((sc) => !query.classifications?.some((qc) => qc.code === sc.code)),
        ];
        // 各特許庁向けクエリ生成
        const offices = query.offices || this.config.defaultOffices;
        const optimizedQueries = new Map();
        const searchUrls = new Map();
        const optimizedQueryObj = {
            ...query,
...
// シングルトンインスタンス
let patentSearchOptimizerInstance = null;
/**
 * PatentSearchOptimizerのシングルトンインスタンスを取得
 */
export function getPatentSearchOptimizer(config) {
 ...
 */
export function resetPatentSearchOptimizer() {
    patentSearchOptimizerInstance = null;
}
//# sourceMappingURL=patent-search-optimizer.js.map