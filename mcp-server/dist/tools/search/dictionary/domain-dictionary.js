...
export class DomainDictionaryManager {
    ...
    expandQuery(query, domain) {
        ...
        const domainsToSearch = detectedDomain
            ? [detectedDomain]
            : enabledDomains;
        for (const d of domainsToSearch) {
            ...
            for (const group of dict.synonymGroups) {
                ...
                const matchedTerm = allTerms.find(term => query.toLowerCase().includes(term.toLowerCase()));
                if (matchedTerm) {
                    // 他の同義語で置換したクエリを追加
                    for (const synonym of allTerms) {
                        if (synonym.toLowerCase() !== matchedTerm.toLowerCase()) {
                            const expandedQuery = query.replace(new RegExp(matchedTerm, 'gi'), synonym);
     ...
     * 多言語展開（日英）
     */
    expandMultilingual(term, domain) {
        const config = getV115FeaturesConfig();
        ...
        for (const group of dict.synonymGroups) {
            ...
            if (allTerms.some(t => t.toLowerCase() === term.toLowerCase())) {
                if (group.english)
                    expanded.add(group.english);
                if (group.japanese)
                    expanded.add(group.japanese);
                break;
            }
        }
        return Array.from(expanded);
     ...
     * 完全なクエリ展開を実行
     */
    async expandQueryFull(query) {
        await this.loadDictionaries();
        const detectedDomain = this.detectDomain(query);
        const expandedQueries = new Set([query]);
        const matchedGroups = [];
        // 同義語展開
                ...
                if (dict) {
                    for (const group of dict.synonymGroups) {
                        ...
                        if (allTerms.some(t => query.toLowerCase().includes(t.toLowerCase()))) {
                            ...
                            if (group.english) {
                                expandedQueries.add(query.replace(new RegExp(group.canonical, 'gi'), group.english));
                                multilingualExpanded = true;
                            }
                            if (group.japanese && group.canonical !== group.japanese) {
                                expandedQueries.add(query.replace(new RegExp(group.canonical, 'gi'), group.japanese));
                                multilingualExpanded = true;
                            }
                        }
                    }
     ...
     * 新語を提案として記録
     */
    suggestNewTerm(term, domain) {
        // 既に辞書にある場合はスキップ
        const dict = this.dictionaries.get(domain);
        if (dict) {
            for (const group of dict.synonymGroups) {
                if (group.canonical.toLowerCase() === term.toLowerCase())
                    return;
                if (group.synonyms.some(s => s.toLowerCase() === term.toLowerCase()))
                    return;
            }
        }
        // 類似用語を検索
 ...
 */
export function getDomainDictionaryManager() {
    if (!_instance) {
 ...
 * シングルトンをリセット（テスト用）
 */
export function resetDomainDictionaryManager() {
    _instance = null;
}
//# sourceMappingURL=domain-dictionary.js.map