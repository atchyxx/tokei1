 ...
 * テキストから数値データを抽出
 */
export function extractNumericData(text, options) {
    ...
    // 信頼度でフィルター
    const filtered = results.filter((r) => r.confidence >= effectiveOptions.minConfidence);
    // 位置順でソート
 ...
 * 日本円を抽出
 */
function extractCurrencyJPY(text, results, options) {
    // 通常の円表記
    ...
    while ((match = pattern1.exec(text)) !== null) {
        ...
        // 単位に応じて乗算
        if (originalText.includes('兆円'))
            value *= UNIT_MULTIPLIERS['兆円'];
        else if (originalText.includes('億円'))
            value *= UNIT_MULTIPLIERS['億円'];
        else if (originalText.includes('万円'))
            value *= UNIT_MULTIPLIERS['万円'];
        results.push({
            originalText,
            value,
            unit: 'JPY',
    ...
    while ((match = pattern2.exec(text)) !== null) {
        ...
        let value = parseFloat(numStr);
        if (originalText.includes('兆'))
            value *= UNIT_MULTIPLIERS['兆'];
        else if (originalText.includes('億'))
            value *= UNIT_MULTIPLIERS['億'];
        else if (originalText.includes('万'))
            value *= UNIT_MULTIPLIERS['万'];
        results.push({
            originalText,
            value,
            unit: 'JPY',
 ...
 * 米ドルを抽出
 */
function extractCurrencyUSD(text, results, options) {
 ...
 * ユーロを抽出
 */
function extractCurrencyEUR(text, results, options) {
 ...
 * 割合を抽出
 */
function extractPercentage(text, results, options) {
 ...
 * 日付を抽出
 */
function extractDates(text, results, options) {
    // 日本語形式
 ...
 * 件数を抽出
 */
function extractCount(text, results, options) {
 ...
 * 測定値を抽出
 */
function extractMeasurement(text, results, options) {
    // 長さ
 ...
 * 比率を抽出
 */
function extractRatio(text, results, options) {
    // 比率（X:Y形式）
...
//# sourceMappingURL=numeric-extractor.js.map