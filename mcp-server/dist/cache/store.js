...
export class FileCacheStore {
     ...
     * クエリからキャッシュキーを生成
     */
    static generateKey(query, source = 'other') {
        return `${source}:${query}`;
    }
    /**
     * クエリハッシュを生成
    ...
    async set(key, value, options = {}) {
        ...
        const ttlSeconds = options.ttlSeconds || this.config.ttlBySource?.[source] || this.config.defaultTtlSeconds;
        const now = new Date();
     ...
     * LRUポリシーでエントリを削除
     */
    async evictLru(targetSizeBytes) {
        await this.ensureInitialized();
        // まず期限切れを削除
        ...
        // 古い順にソート
        candidates.sort((a, b) => new Date(a.lastAccessedAt).getTime() - new Date(b.lastAccessedAt).getTime());
        let evictedCount = 0;
        let freedBytes = 0;
     ...
     * ソース別統計を更新
     */
    updateSourceStats(source, action, size) {
        ...
        const sourceStats = this.stats.bySource[source];
        switch (action) {
            case 'hit':
                sourceStats.hits++;
                break;
